...
export default function AdminTransactions() {
  ...
  const loadTransactions = useCallback(
    async (nextPage = 1) => {
      try {
        ...
        setError(
          normalizeError(err, "We could not load transactions.")
        );
      } finally {
        setLoading(false);
      }
    },
    [search, status, feeType, faculty]
  );
  ...
  async function handleVerify(transactionId) {
    try {
      ...
      setError(
        normalizeError(
          err,
          "We could not verify that transaction."
        )
      );
    } finally {
      setBusyId("");
  ...
  return (
    <div className="cp-admin-page">
      <div className="cp-admin-page__header">
        <div>
          ...
          <p className="cp-admin-page__sub">
            Filter, verify, and inspect payment activity across the platform.
          </p>
        </div>
      </div>
      ...
      <form
        className="cp-admin-controls cp-admin-controls--grid"
      ...
      >
        <input
          ...
          placeholder="Search by receipt or matric"
        />
              ...
              <tbody>
                {transactions.map((transaction) => (
                  <tr key={transaction.id}>
                    ...
                    <td>
                      {FEE_LABELS[transaction.fee_type] ||
                    ...
                    <td>
                      <span
                        className={`cp-pill cp-pill--${transaction.status}`}
                      >
                        {transaction.status}
                      </span>
                    </td>
