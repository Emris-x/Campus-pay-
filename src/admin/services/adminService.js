...
export async function fetchAdminOverview() {
  ...
  const totalVolume = transactionRows.reduce(
  ...
  const verifiedCount = transactionRows.filter(
    (row) => row.status === "verified"
  ).length;

  const failedCount = transactionRows.filter(
...
export async function fetchUsers({
  ...
  let query = supabase
    .from("students")
    .select(
      "id, full_name, email, matric_number, registration_number, faculty, created_at",
      { count: "exact" }
    );
...
export async function fetchTransactions({
  ...
  if (faculty !== "all") {
    query = query.eq("faculty_name", faculty);
  }
...
/**
 * Verifies a transaction after the admin has confirmed
...
export async function fetchPaymentsSummary() {
  const {
    ...
    .from("transactions")
    .select("id, amount, fee_type, status");
  ...
  if (error) {
    if (isTableMissingError(error)) {
      return {
        unavailable: true,
        message:
          "Wallet ledger data is not available until the admin migration is applied.",
      };
    }
...
export async function fetchAnalytics() {
  const {
    ...
    .from("transactions")
    .select(
      "id, amount, status, created_at, fee_type, faculty_name"
    );
  ...
  const volumeByMonth = rows.reduce((acc, row) => {
    const month = new Date(row.created_at).toLocaleString(
      "en-US",
      { month: "short" }
    );

    acc[month] =
      (acc[month] || 0) + Number(row.amount || 0);
...
export async function fetchNotifications() {
  const {
    ...
    .from("notifications")
    .select(
      "id, title, body, type, status, created_at"
    )
    .order("created_at", { ascending: false })
    .limit(10);
  ...
  if (error) {
    if (isTableMissingError(error)) {
      return {
        unavailable: true,
        message:
          "Notifications are wired into the schema migration, but the current database has not applied it yet.",
      };
    }
...
export async function fetchAdmins() {
  const {
    ...
    .from("admin_profiles")
    .select(
      "id, role, status, created_at, last_seen_at"
    )
    .order("created_at", { ascending: false });
  ...
  if (error) {
    if (isTableMissingError(error)) {
      return {
        unavailable: true,
        message:
          "Admin profiles are not available until the migration is applied.",
      };
    }
  ...
  if (error) {
    if (isTableMissingError(error)) {
      return {
        unavailable: true,
        message:
          "Audit trails will appear here once the admin migration is applied to Supabase.",
      };
    }
  ...
  if (error) {
    if (isTableMissingError(error)) {
      return {
        unavailable: true,
        message:
          "System settings are ready in the migration plan and will become available after the database is updated.",
      };
    }
...
export async function recordAuditEvent({
  ...
  if (error) {
    if (isTableMissingError(error)) {
      throw new Error(
        "Audit logs are not available until the Supabase migration is applied."
      );
    }
